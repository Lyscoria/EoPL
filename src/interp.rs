use crate::{ast::{Exp, Program}, env::Env, err::RuntimeError, store::Store, val::{ExpVal, Proc}};
...
pub fn value_of_program(program: &Program) -> Result<ExpVal, RuntimeError> {
    ...
    value_of(&program.exp, &init_env, &mut store)
}

pub fn value_of(exp: &Exp, env: &Env, store: &mut Store) -> Result<ExpVal, RuntimeError> {
    match exp {
        ...
        Exp::DiffExp(exp1, exp2) => {
            let val1 = value_of(exp1, env, store)?;
            let val2 = value_of(exp2, env, store)?;
        ...
        Exp::ListExp(exps) => {
            ...
            for exp in exps.iter().rev() {
                val = ExpVal::cons(value_of(exp, env, store)?, val)?;
            }
            Ok(val)
        }
        ...
        Exp::IfExp(exp1, exp2, exp3) => {
            let val = value_of(exp1, env, store)?.as_bool()?;
            if val {
                value_of(exp2, env, store)
            } else {
                value_of(exp3, env, store)
            }
        }

        Exp::CondExp(clauses) => {
            for (test, res) in clauses {
                let val = value_of(test, env, store)?;
                if val.as_bool()? == true {
                    return value_of(res, env, store);
                }
            }
        ...
        Exp::LetStarExp(bindings, body) => {
            let mut new_env = env.clone();
            for (var, exp) in bindings {
                let val = value_of(exp, &new_env, store)?;
                new_env = new_env.extend(var.to_string(), val);
            }
            value_of(body, &new_env, store)
        }
        ...
        Exp::UnpackExp(vars, exp, body) => {
            let vals = value_of(exp, env, store)?;
        ...
        Exp::CallExp(rator, rands) => {
            let proc = value_of(rator, env, store)?.as_proc()?;
            let mut args = Vec::new();
            for rand in rands {
                args.push(value_of(rand, env, store)?);
            }
            ...
            value_of(&proc.body, &new_env, store)
        }
        ...
        Exp::LetProcExp(name, vars, body, let_body) => {
            ...
            value_of(let_body, &new_env, store)
        }
        ...
        Exp::BeginExp(exps) => {
            let mut val = ExpVal::Int(0);
            for exp in exps {
                val = value_of(exp, env, store)?;
            }
            Ok(val)
        }
