...
use eopl::translator::translate_program;
use eopl::nameless_interp::value_of_program;
    ...
    match expected {
        Expected::Error => {
            let is_err = if let Ok(p) = program {
                if let Ok(np) = translate_program(p) {
                    value_of_program(&np).is_err()
                } else {
                    true 
                }
            } else {
        ...
        Expected::Num(n) => {
            ...
            let np = translate_program(p).unwrap_or_else(|e| {
                panic!("Translate error: {:?}\nSource: {}", e, source)
            });

            let res = value_of_program(&np).unwrap_or_else(|e| {
                panic!("Runtime error: {:?}\nSource: {}", e, source)
            });
