...
eopl_test!(begin_test_1, "begin 1; 2; 3 end", 3);

eopl_test!(gensym_test_1, "
let g = let counter = newref(0) 
         in proc (dummy) let d = setref(counter, -(deref(counter), -1))
                    in deref(counter)
in -((g 11),(g 22))", -1);

eopl_test!(simple_store_test_1, "let x = newref(17) in deref(x)", 17);

eopl_test!(assignment_test_1, "
let x = newref(17) 
in begin setref(x,27); deref(x) end", 27);

eopl_test!(gensym_test_2, "
let g = let counter = newref(0) 
         in proc (dummy) begin
